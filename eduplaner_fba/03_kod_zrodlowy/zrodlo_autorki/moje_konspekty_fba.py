...
.mkfcard{max-width:940px; margin:0 auto; background:var(--paper); border-radius:14px;
  padding:21px 23px 19px; position:relative; box-shadow:0 18px 60px rgba(36,28,58,.35)}
/* Krzyżyk taki sam jak w konspekcie — nauczycielka szuka go w prawym górnym
   rogu okna, niezależnie od tego, czy konspekt czyta, czy pisze. Zamyka
   z pytaniem, bo Escape nie może kasować niezapisanej pracy. */
.mkf-close{position:absolute; top:14px; right:16px; border:1px solid var(--line-2);
  background:var(--paper); color:var(--ink); border-radius:8px; width:30px; height:30px;
  cursor:pointer; font:600 13px/1 inherit}
.mkf-close:hover{border-color:var(--accent); color:var(--accent)}
.mkf-head{display:flex; align-items:baseline; gap:12px; padding:0 40px 10px 0;
...
<div class="mkfmodal" id="mkf-edytor" role="dialog" aria-modal="true"
  ...
  <div class="mkfcard">
    <button type="button" class="mkf-close" id="mkf-zamknij"
      aria-label="Zamknij edytor" title="Zamknij edytor">✕</button>
    <div class="mkf-head">
...
(function(){
  ...
  document.getElementById('mkf-anuluj').addEventListener('click',wyjdzZEdytora);
  document.getElementById('mkf-zamknij').addEventListener('click',wyjdzZEdytora);
