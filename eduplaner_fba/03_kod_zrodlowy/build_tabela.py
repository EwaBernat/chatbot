#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Druk FBA-T — tabela 225 celów SMART, z której wchodzi się w konspekty.
...
WYJSCIE = KORZEN / "02_gotowe_dokumenty" / "Tabela_celow_FBA_wiek_poziom.html"
...
def naglowek(liczba_celow: int) -> str:
  ...
  <div><h1>EduPlaner 2026</h1>
    <div class="sub">Analiza funkcjonalna i plan PBS · tabela celów SMART · wiek i poziom wsparcia</div></div>
  <div class="prawa"><b>FBA · WOPF</b><span>narzędzie · druk FBA-T</span></div>
</div>
<div class="kreska"></div>
<div class="tyt">
  <span class="pigula">{liczba_celow} celów SMART</span>
  <p>25 wskaźników kwestionariusza funkcji zachowania — pięć funkcji po pięć pozycji — razy
     trzy wersje wiekowe i trzy poziomy wsparcia. Wiersz mówi, <b>czego dziecko uczy się robić
     zamiast zachowania trudnego</b>, żeby załatwić tę samą potrzebę; kolumna — ile przy tym
     dostaje podpory. Kliknięcie w cel otwiera konspekt zajęć z wyróżnionym tym poziomem.</p>
</div>"""
...
def uwaga(opis_modulu: str) -> str:
    return f"""<div class="uwaga"><b>Poziom zmienia warunki, nie zachowanie zastępcze.</b>
...
def wykaz_konspektow(wersja: str, funkcje: list[dict], konspekty_wg: dict) -> str:
    grupy = []
    for z in funkcje:
        karty = "".join(
            ...
            f'<span class="kzast">{e(w["zachowanie_zastepcze"])}</span></button>'
            for w in z["wskazniki"])
        grupy.append(f'<div class="kgrupa"><h4>Funkcja {e(z["nr"])} · {e(z["nazwa"])} '
...
def tabela_wersji(wersja: str, wiek: str, funkcje: list[dict], poziomy: list[dict],
                  konspekty_wg: dict) -> str:
    wiersze = []
    for z in funkcje:
        wiersze.append(f'<tr class="pas"><td colspan="5">Funkcja {e(z["nr"])} · {e(z["nazwa"])}'
        ...
        for w in z["wskazniki"]:
            ...
            wiersze.append(
                f'<tr data-wsk="{e(w["nr"])}" data-funkcja="{e(z["nr"])}" '
                f'data-krok="{e(w["zachowanie_zastepcze"])}">'
                f'<td class="nr">{e(w["nr"])}</td>'
                f'<td class="wsk"><b>{e(w["wskaznik"])}</b>'
                f'<span>zachowanie zastępcze: {e(w["zachowanie_zastepcze"])}</span>'
                f'<span>pozycja kwestionariusza: {e(w["pozycja"])} · skala 0–3</span>'
  ...
  {wykaz_konspektow(wersja, funkcje, konspekty_wg)}
  <table>
    ...
    <thead>
      <tr class="wband"><th colspan="5">EduPlaner 2026 · druk FBA-T · cele SMART do wskaźników
        rozwoju FBA <b>wersja {wersja} · {e(wiek)}</b></th></tr>
      <tr><th>Nr</th><th>Wskaźnik z karty obserwacji FBA</th>{naglowki}</tr>
    </thead>
  ...
  <div class="kcard">
    ...
    <div class="khead"><span class="mark" role="img" aria-label="Logo PCTP"></span>
      <div><div class="kw">EduPlaner 2026</div>
        <div class="ks">Konspekt · funkcja {e(kon['funkcja'])} · pozycja {e(kon['pozycja'])} · wersja
          {e(kon['wersja_wiekowa'])} · {e(kon['wiek'])} · wskaźnik {e(kon['wskaznik'])}</div></div>
      <span class="kpill">Konspekt FBA {e(kon['wersja_wiekowa'])}-{e(kon['wskaznik'])}</span></div>
    ...
    <div class="kkrok"><b>Zachowanie zastępcze:</b> {e(wskaznik['zachowanie_zastepcze'])}
      <span>{e(wskaznik['opis_kroku'])}</span></div>

    <div class="ksec"><span class="sq">I</span><h4>Cel SMART</h4><span class="line"></span></div>
    <div class="kcele">
      <div class="kcel edu"><div class="kchead">Cel edukacyjny — z tabeli FBA-T, wg klikniętego poziomu</div>
        {warianty}</div>
    ...
    <div class="kdwie">
      ...
      <div class="kwsk" style="border-left-color:var(--pomarancz);background:var(--pomarancz-tlo)">
        <b>Zasada PBS:</b> {e(kon['zasada_pbs'])}</div></div>
...
const FUNKCJAY_JS = __FUNKCJAY__;
...
function kontekst(td){
  ...
  return {wersja: td.dataset.wersja, nr: td.dataset.wsk, poziom: td.dataset.lvl,
          funkcja: tr.dataset.funkcja, krok: tr.dataset.krok,
...
function formularz(k, rekord){
  ...
  const m = modal('mkt-form', `
    ...
    <div class="khead"><span class="mark" role="img" aria-label="Logo PCTP"></span>
      <div><div class="kw">EduPlaner 2026</div>
        <div class="ks">Mój konspekt · funkcja ${esc(k.funkcja)} · ${esc(FUNKCJAY_JS[k.funkcja] || '')}
    ...
    <div class="kkrok"><b>Cel z tabeli (${esc(poz.nazwa)}):</b> ${esc(t.cel)}
      <span>${esc(t.ram)} · zachowanie zastępcze: ${esc(k.krok)}</span></div>
  ...
  m.querySelector('#mkt-formularz').addEventListener('submit', ev => {
    ...
    const rek = {
      ...
      nr: k.nr, wersja: k.wersja, poziom: k.poziom, funkcja: k.funkcja, krok: k.krok,
...
function pokazWlasny(id){
  ...
  modal('mkt-widok', `
    ...
    <div class="khead"><span class="mark" role="img" aria-label="Logo PCTP"></span>
      <div><div class="kw">EduPlaner 2026</div>
        <div class="ks">Konspekt własny · funkcja ${esc(r.funkcja)} · ${esc(FUNKCJAY_JS[r.funkcja] || '')}
    ...
    <div class="ktitle"><span class="kp">Konspekt zajęć · druk KC-3 · opracowanie własne</span>
      <div class="ksfera">Wskaźnik ${esc(r.nr)} · zachowanie zastępcze: ${esc(r.krok)}</div>
    ...
    <div class="ksec"><span class="sq">I</span><h4>Cel SMART</h4><span class="line"></span></div>
    <div class="kcele">
      <div class="kcel edu"><div class="kchead">Cel edukacyjny — z tabeli FBA-T · ${esc(poz.nazwa)}</div>
  ...
  document.getElementById('mkt-edytuj').addEventListener('click', () => {
    zamknij();
    formularz({wersja: r.wersja, nr: r.nr, poziom: r.poziom, funkcja: r.funkcja,
...
def main() -> int:
    cele = wczytaj("cele_fba_poziomy.json")
    konspekty = wczytaj("konspekty_fba.json")
    pomoce = wczytaj("pomoce_fba.json")
    materialy = wczytaj("materialy_do_druku.json")

    poziomy, wersje, funkcje = cele["poziomy_wsparcia"], cele["wersje_wiekowe"], cele["funkcje"]
    ...
    wskaznik_wg = {w["nr"]: w for z in funkcje for w in z["wskazniki"]}

    sekcje = "".join(tabela_wersji(w["klucz"], w["wiek"], funkcje, poziomy, konspekty_wg) for w in wersje)
    modale = "".join(
    ...
    skrypt = (SKRYPT
              .replace("__POZIOMY__", json.dumps({p["klucz"]: {"nazwa": p["nazwa"], "rzym": p["rzym"],
                                                 ...
                                                 ensure_ascii=False))
              .replace("__FUNKCJAY__", json.dumps({z["nr"]: z["nazwa"] for z in funkcje}, ensure_ascii=False))
...
<title>Tabela celów SMART · rozwój FBA (przedszkole) — EduPlaner 2026 · PCTP</title>
...
{sekcje}
  <div class="stopka">
    ...
    <span>druk FBA-T · tabela drukuje się poziomo · {cele['liczba_celow']} celów ·
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Tabela celów FBA z konspektami")
