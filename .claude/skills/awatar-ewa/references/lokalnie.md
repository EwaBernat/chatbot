...
git fetch origin claude/ewy-character-skill-avatar-1tdd3m
git checkout claude/ewy-character-skill-avatar-1tdd3m      # po scaleniu do main: git checkout main
```

W `.claude/skills/` masz wtedy komplet: `awatar-ewa` (render filmu z Twoim awatarem),
`awatar-ewa-pctp` (postać Ewa PCTP: karta, wycięta postać, plansze, prezentacje, Remotion),
`dane-i-glos` (Twój głos z ElevenLabs, filmy z danych) oraz oficjalne `heygen-video`,
`heygen-avatar` i `heygen-translate`.

Skrypty postaci potrzebują kilku bibliotek (jednorazowo):
...
`avatar_id` i `voice_id` — przydadzą się w kroku 4b. `heygen voice list` jest potrzebne
tylko wtedy, gdy Ewa ma mówić głosem z konta HeyGen; z klonem w ElevenLabs wystarczy
sam `avatar_id`.

Zapamiętaj awatara w pamięci skilli (jeden raz, u siebie):

```bash
python3 .claude/skills/awatar-ewa-pctp/scripts/zapamietaj_awatara.py --avatar-id <avatar_id> --nazwa "Ewa PCTP"
python3 .claude/skills/awatar-ewa-pctp/scripts/zapamietaj_awatara.py --pokaz
```

Od tej chwili `awatar-ewa`, `awatar-ewa-pctp` i `dane-i-glos` biorą Ewę z pamięci. Pusta lista awatarów prywatnych znaczy,
