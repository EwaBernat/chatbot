...
## Postać „Ewa PCTP" — skill `awatar-ewa-pctp`

Ten skill **renderuje** film, w którym Ewa mówi. Wszystko, co dotyczy samej postaci,
prowadzi skill `awatar-ewa-pctp` i tam sięgaj, gdy prośba brzmi „wstaw Ewę", „daj mi
samą postać", „Ewa w rogu prezentacji", „Ewa w Remotion":

- karta postaci (wygląd, kadr, strój, gest, ton, tekst intro) — `references/postac.md`,
- gotowa wycięta Ewa bez tła: `assets/ewa_pctp.png` i `assets/ewa_pctp_intro.webm`,
- `wytnij_postac.py` (sama postać z klipu), `wstaw_ewe.py` (na planszę, ekran, film),
  `ewa_do_prezentacji.py` (PPTX), `--awatar` w `przygotuj_remotion.py`.

Gdy render z tego skilla ma potem trafić na planszę, slajd albo do Remotion, zamów go
**na jednolitym zielonym tle** (`--tlo "#00FF00"` w skrypcie REST; w promptcie do Video
Agenta: „tło jednolite, czysta zieleń #00FF00, bez gradientu") albo wyeksportuj z HeyGen
jako WebM z przezroczystym tłem. Fiolet marki jako tło wyklucza późniejsze wycięcie —
marynarka Ewy ma ten sam odcień.

Pamięć awatara jest wspólna: `skonfiguruj_awatara.py` stąd i `zapamietaj_awatara.py`
z `awatar-ewa-pctp` piszą do tego samego pliku i tych samych pól.
