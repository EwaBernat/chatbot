...
# Układ dwudziestu pól. 'e' = spotkanie (nazwę emocji podaje wywołujący),
# 'w' = pole z wieńcem, 'p' = przerwa. Kolejność jest stała dla całej serii.
UKLAD = ['start', 'e', 'e', 'w', 'e', 'e', 'p', 'e', 'e', 'w',
         'e', 'e', 'p', 'e', 'e', 'w', 'e', 'e', 'w', 'meta']


def uloz_pola(emocje):
    """emocje: lista dwunastu nazw emocji, po jednej na rozdział."""
    if len(emocje) != 12:
        raise ValueError('Plansza potrzebuje dokładnie 12 emocji, dostała %d' % len(emocje))
    pola, nr = [], 0
    for typ in UKLAD:
        if typ == 'e':
            nr += 1
            pola.append(('e', str(nr), emocje[nr - 1]))
        elif typ == 'start':
            pola.append(('start', 'START', ''))
        elif typ == 'meta':
            pola.append(('meta', 'META', ''))
        else:
            pola.append((typ, '', ''))
    return pola
