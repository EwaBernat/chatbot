...
function liniePisania(n) {
  const out = [];
  for (let i = 0; i < (n || 2); i++) out.push(new Paragraph({
    children: [T(' ', { size: 14 })], spacing: { after: 90 },
    border: { bottom: { style: BorderStyle.DOTTED, size: 6, color: KROPKI } } }));
  return out;
}
function wierszeSmart(lista) {
  return lista.map(w => new Paragraph({
    children: [T(w.l + '  ', { bold: true, size: 14, color: POMARANCZ }),
               T(w.op + '   ', { italics: true, size: 12, color: SZARY })],
    spacing: { after: 80 },
    border: { bottom: { style: BorderStyle.DOTTED, size: 6, color: KROPKI } } }));
}
function bCele(b) {
  const w = rozdziel(SZER, b.cele.map(() => 1));
  return tabela([new TableRow({ children: b.cele.map((c, i) => {
    const d = [P([T((c.znak ? c.znak + '  ' : '') + c.tytul.toUpperCase(),
                    { bold: true, size: 14, color: FIOLET, spacing: 6 })], { po: 30 })];
    if (c.pig)  d.push(P([T(c.pig, { size: 11, color: POMARANCZ, bold: true })], { po: 40 }));
    if (c.mini) d.push(P([T(c.mini, { size: 11, color: SZARY, italics: true })], { po: 40 }));
    d.push(...liniePisania(2));
    if (c.smart && c.smart.length) d.push(...wierszeSmart(c.smart));
    if (c.kryt) d.push(P([T(c.kryt, { bold: true, size: 12, color: FIOLET })], { po: 0 }));
    return komorka(d, { w: w[i], tlo: POLE, mt: 110, mb: 110 });
  }) })], w, {});
}
function bSmart(b) { return wierszeSmart(b.wiersze); }
function bZleDobrze(b) {
  const w = rozdziel(SZER, [1, 1]);
  const kom = [[b.zle, 'FDECE4', POMARANCZ], [b.dobrze, 'EDF7EE', '2E7D32']].map(function (para, i) {
    const x = para[0], tlo = para[1], kol = para[2];
    return komorka([P([T((x.etyk || '').toUpperCase(), { bold: true, size: 11, color: kol, spacing: 6 })], { po: 40 }),
                    P([T(x.tresc || '', { size: 13 })], { po: 0, linia: 260 })],
                   { w: w[i], tlo: tlo, mt: 110, mb: 110 });
  });
  return tabela([new TableRow({ children: kom })], w, {});
}
function bArgumenty(b) {
  const out = [];
  b.arg.forEach(function (a) {
    out.push(tabela([new TableRow({ children: [komorka([
      P([T(a.tytul, { bold: true, size: 14, color: FIOLET })], { po: 40 }),
      P(biegi(a.tresc, { size: 13 }), { po: 0, linia: 260 })
    ], { w: SZER, tlo: POLE, mt: 110, mb: 110 })] })], [SZER], {}));
    out.push(P([T('')], { po: 80 }));
  });
  return out;
}
function bFormula(b) {
  const w = rozdziel(SZER, b.czlony.map(() => 1));
  const tla = ['2D1B69', '4A2E86', '6A4C9E', 'E8712F', 'E8450A'];
  return tabela([new TableRow({ children: b.czlony.map((c, i) => komorka([
    P([T(c.b, { bold: true, size: 12, color: 'FFFFFF' })], { align: AlignmentType.CENTER, po: 30 }),
    P([T(c.i, { size: 10, color: 'FFFFFF', italics: true })], { align: AlignmentType.CENTER, po: 0 })
  ], { w: w[i], tlo: tla[i % tla.length], mt: 100, mb: 100 })) })], w, {});
}
function bChipsy(b) {
  const w = rozdziel(SZER, b.chipy.map(() => 1));
  return tabela([new TableRow({ children: b.chipy.map((c, i) => komorka([
    P([T(c.toUpperCase(), { bold: true, size: 10, color: FIOLET, spacing: 6 })], { align: AlignmentType.CENTER, po: 40 }),
    new Paragraph({ children: [T(' ', { size: 12 })], spacing: { after: 0 },
      border: { bottom: { style: BorderStyle.DOTTED, size: 6, color: KROPKI } } })
  ], { w: w[i], tlo: POLE, mt: 90, mb: 90 })) })], w, {});
}
function bWsk(b) {
  return new Paragraph({ children: [T(b.tresc, { bold: true, size: 12, color: FIOLET })],
    spacing: { before: 120, after: 60 },
    border: { bottom: { style: BorderStyle.DOTTED, size: 6, color: KROPKI } } });
}
function bDuo(b) {
  const w = rozdziel(SZER, b.kolumny.map(() => 1));
  return tabela([new TableRow({ children: b.kolumny.map(function (kol, i) {
    const d = [];
    kol.forEach(function (x) { const e = element(x); if (e) d.push(...(Array.isArray(e) ? e : [e])); });
    if (!d.length) d.push(P([T('')], { po: 0 }));
    return komorka(d, { w: w[i], bez: true, mt: 0, mb: 0 });
  }) })], w, { bez: true });
}
...
function element(b) {
  switch (b.typ) {
    ...
    case 'cele':        return bCele(b);
    case 'smart':       return bSmart(b);
    case 'zle-dobrze':  return bZleDobrze(b);
    case 'argumenty':   return bArgumenty(b);
    case 'formula':     return bFormula(b);
    case 'chipsy':      return bChipsy(b);
    case 'wsk':         return bWsk(b);
    case 'duo':         return bDuo(b);
    default:            return null;
  }
}
...
function x_potrzebuje_odstepu(b) {
  return ['tabela','pole','siatka','wybory','prawna','legenda','info','blok-dyr','miesiace','karty','kafle','cele','smart','zle-dobrze','argumenty','formula','chipsy','duo'].includes(b.typ);
}
