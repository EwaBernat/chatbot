...
const MARGINS = { top: 1000, right: 850, bottom: 1100, left: 850 };
...
const pageHeader = (caption) => [
  tbl([700, 6200, 3006], [ row([
    ...
    cell([ P([ run('[Nazwa przedszkola / szkoły]', { size:30, bold:true, color:C.purple }) ], { after:20 }), P([ run(caption, { size:18, color:C.muted, caps:true, spacing:12 }) ], { after:0 }) ], { width:6200, borders:noBorders, vAlign:VerticalAlign.CENTER, margins:{ top:0, bottom:0, left:160, right:0 } }),
  ...
  new Paragraph({ spacing:{ before:60, after:160 }, border:{ bottom:{ style:BorderStyle.SINGLE, size:20, color:C.purple, space:1 } }, children:[] }),
  // lawendowe pola
  tbl([3900, 2900, 2906], [ row([
    ['DOTYCZY DZIECKA', 3900], ['GRUPA / KLASA', 2900], ['DATA', 2906]
  ].map(([t,w]) => cell(P([ run(t, { size:20, bold:true, color:C.lavText, spacing:12 }), run('  ' + '.'.repeat(w===3900?40:22), { size:20, color:'B6A6DF' }), run(t==='DATA'?'  r.':'', { size:20, color:C.muted }) ], { after:0 }), { width:w, bg:C.lav, borders:{ top:ln(C.white,12), bottom:ln(C.white,12), left:ln(C.white,12), right:ln(C.white,12) }, margins:{ top:110, bottom:110, left:180, right:120 } })) ) ]),
  empty(120)
];
...
const section = (n, title) => new Paragraph({
  ...
  border:{ bottom:{ style:BorderStyle.SINGLE, size:20, color:C.line, space:6 } },
...
const cover = [
  ...
  P([ run('  OPINIA PRZEDSZKOLA / SZKOŁY · DLA ZESPOŁU ORZEKAJĄCEGO · DLA RODZICA  ', { size:20, bold:true, color:C.white, bg:C.orange, spacing:12 }) ], { align:AlignmentType.CENTER, before:40, after:120 }),
  ...
  ...(() => {
    ...
    const out = [];
    const head = (t) => P([ run('■ ', { size:20, color:C.purple }), run(t.toUpperCase(), { size:20, bold:true, color:C.purple, spacing:14 }) ], { after:70 });
    out.push(head(T.I.title));
    out.push(tbl([3302,3302,3302], [ row(T.I.items.map(t => cell([ P([ run(t[0], { size:30, bold:true, color:C.orange }) ], { after:20 }), P([ run(t[1], { size:24, bold:true, color:C.purple }) ], { after:30 }), P([ run(t[2], { size:20, color:C.muted }) ], { after:0, line:230 }) ], { width:3302, borders:{ top:{ style:BorderStyle.SINGLE, size:30, color:C.orange }, bottom:ln(), left:ln(), right:ln() }, margins:{ top:80, bottom:90, left:160, right:160 } }))) ]));
    for (const part of ['II','III']) {
      ...
      out.push(tbl(ws, [ row(T[part].items.map((t,i) => cell([ P([ run(t[0], { size:24, bold:true, color:C.orange }) ], { after:10 }), P([ run(t[1], { size:20, bold:true, color:C.purple }) ], { after:0, line:210 }) ], { width:ws[i], borders:{ top:{ style:BorderStyle.SINGLE, size:30, color:C.orange }, bottom:ln(), left:ln(), right:ln() }, margins:{ top:60, bottom:70, left:90, right:60 } }))) ]));
    }
    return out;
  })(),
...
const metaRow = (lbl, valueChildren) => row([
  cell(P([ run(lbl, { size:20, bold:true, color:C.purple, caps:true, spacing:10 }) ], { after:0 }), { width:LW, bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:100, bottom:100, left:180, right:120 } }),
...
const icfRow = tbl([1981,1981,1981,1981,1982], [ row([
  ...
  P([ run(t[1], { size:20, bold:true, color:C.lavText }) ], { align:AlignmentType.CENTER, after:0 })
...
const toolCell = (tag, title, desc, color, width, extra=[]) => cell([
  P([ run(tag.toUpperCase(), { size:20, bold:true, color, spacing:14 }) ], { after:40 }),
...
const abcRow = tbl([3100,3100,3100], [ row([
  ...
  P([ run(t[0], { size:30, bold:true, color:C.red }) ], { align:AlignmentType.CENTER, after:10 }),
...
const sigCell = (role, width, span, top=300) => new TableCell({
  ...
  children:[
    new Paragraph({ alignment:AlignmentType.CENTER, spacing:{ before:0, after:0 }, border:{ top:{ style:BorderStyle.SINGLE, size:20, color:C.purple, space:5 } }, children:[ run(role, { size:24, bold:true, color:C.purple }) ] }),
    new Paragraph({ alignment:AlignmentType.CENTER, spacing:{ before:10, after:0 }, children:[ run('podpis i data', { size:20, color:C.muted }) ] })
  ]
});
...
const footerPara = new Paragraph({
  spacing:{ before:60, after:0 }, border:{ top:{ style:BorderStyle.SINGLE, size:20, color:C.line2, space:4 } },
  tabStops:[{ type:TabStopType.RIGHT, position:CW }],
  children:[ run('[Nazwa placówki] · dokument poufny (RODO)', { size:20, color:C.muted }), run('   · sporządzono w EduPlaner 2026', { size:20, color:'B6A6DF' }), run('\t'), run('Strona ', { size:20, color:C.muted }), new TextRun({ children:[PageNumber.CURRENT], font:FONT, size:20, bold:true, color:C.orange }), run(' z ', { size:20, color:C.muted }), new TextRun({ children:[PageNumber.TOTAL_PAGES], font:FONT, size:20, bold:true, color:C.purple }), run(' · Ocena Funkcjonalna', { size:20, color:C.muted }) ]
});


// =============== CZĘŚĆ II–III · DANE Z raport_data.json ===============
const lawP = (law) => law ? P([ run(' § Podstawa prawna: '+law+' ', { size:20, bold:true, color:C.purple, bg:C.lav }) ], { before:0, after:120 }) : empty(0);
...
const pb = () => new Paragraph({ pageBreakBefore:true, spacing:{ before:0, after:0, line:20, lineRule:'exact' }, children:[ run('', { size:20 }) ] });
const gridHead = (cols, widths, colors=[]) => row(cols.map((t,i) => cell(P([ run(t, { size:20, bold:true, color:colors[i]||C.purple, caps:true, spacing:10 }) ], { after:0 }), { width:widths[i], bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:100, bottom:100, left:140, right:120 } })));
const gcell = (children, width, o={}) => cell(Array.isArray(children)?children:[children], { width, bg:o.bg, borders:{ top:ln(C.line2), bottom:ln(C.line2), right:ln(C.line2), left: o.edge ? { style:BorderStyle.SINGLE, size:30, color:o.edge } : ln(C.line2) }, margins:{ top:o.pad??70, bottom:o.pad??70, left:140, right:120 } });
const txt = (t, o={}) => P([ run(t, { size:o.size||24, bold:o.bold, color:o.color, italic:o.italic }) ], { after:0, line:o.size&&o.size<20?250:270 });
...
const sub9 = (tag, title, note, color) => new Paragraph({ spacing:{ before:140, after:90 }, keepNext:true, children:[ run(' '+tag+' ', { size:24, bold:true, color:C.white, bg:color }), run('   '+title, { size:24, bold:true, color:C.purple }), run(note ? '   · '+note : '', { size:20, color:C.muted }) ] });
const bandW = (part, title, sub) => [
  P([ run('  OCENA FUNKCJONALNA DZIECKA / UCZNIA · PODSUMOWANIE WOPF I IPET · CZĘŚĆ '+part+'  ', { size:20, bold:true, color:C.white, bg: part==='III' ? C.purple : C.orange, spacing:10 }) ], { align:AlignmentType.CENTER, after:120 }),
  P([ run(title, { size:30, bold:true, color:C.purple }) ], { align:AlignmentType.CENTER, after:60, line:300 }),
  P([ run(sub.toUpperCase(), { size:20, bold:true, color:C.orange, spacing:30 }) ], { align:AlignmentType.CENTER, after:60 })
];
const chk = (on) => run(on ? '☑  ' : '☐  ', { size:24, color: on ? C.orange : 'B6A6DF' });
const labelP = (t) => P([ run(t, { size:20, bold:true, color:C.purple, caps:true, spacing:12 }) ], { before:60, after:60 });
...
const statTile = (l, v, sub, width, top=C.orange) => cell([
  P([ run(l, { size:20, bold:true, color:C.lavText, spacing:12 }) ], { after:30 }),
  P([ run(v, { size:30, bold:true, color:C.purple }), run(sub ? '  '+sub : '', { size:20, color:C.muted }) ], { after:0 })
], { width, borders:{ top:{ style:BorderStyle.SINGLE, size:30, color:top }, bottom:ln(), left:ln(), right:ln() }, margins:{ top:70, bottom:70, left:160, right:120 } });
...
const part5 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Wyniki liczbowe'),
  ...secL('5', s5.title, s5.law), lead2(s5.ref, s5.lead),
  tbl([CW], [ row([ cell(P([ run('NARZĘDZIE BAZOWE:  ', { size:20, bold:true, color:C.lavText, spacing:12 }), chk(false), run('Przedszkole · KPOF', { size:24, bold:true, color:C.purple }), run('  (bez stenów)', { size:20, color:C.muted }), run('        ', { size:24 }), chk(false), run('Szkoła · KSzOF', { size:24, bold:true, color:C.purple }), run('  (ze stenami)', { size:20, color:C.muted }) ], { after:0 }), { width:CW, bg:C.lav, borders:noBorders, margins:{ top:70, bottom:70, left:180, right:180 } }) ]) ]),
  empty(60),
  ...
  tbl(W5, [ gridHead(['Kod','Domena ICF','Punkty / śr.','Sten (KSzOF)','Poziom wsparcia','Wskaźnik funkcjonalny'], W5),
    ...s5.dom.map(d => row([ gcell(txt(d[0], { bold:true, color:C.orange }), W5[0]), gcell(txt(d[1], { bold:true }), W5[1]), gcell(txt('Śr: '+String(d[2]).replace('.',',')+' ('+d[3]+' pkt)'), W5[2]), gcell(txt('Sten '+d[4]), W5[3]), gcell(P([ run(' Poziom '+d[5]+' · '+d[6]+' ', { size:20, bold:true, color:LVL[d[5]].fg, bg:LVL[d[5]].bg }) ], { after:0 }), W5[4]), gcell(txt(d[7]), W5[5]) ])) ]),
  P([ run('Poziom 1 · Niski', { size:20, bold:true, color:LVL[1].fg, bg:LVL[1].bg }), run('    ', { size:20 }), run('Poziom 2 · Średni', { size:20, bold:true, color:LVL[2].fg, bg:LVL[2].bg }), run('    ', { size:20 }), run('Poziom 3 · Wysoki', { size:20, bold:true, color:LVL[3].fg, bg:LVL[3].bg }) ], { align:AlignmentType.RIGHT, before:100, after:0 }),
  P([ run('Zasada: ', { size:20, bold:true, color:C.purple }), run('KPOF (przedszkole) nie posiada norm stenowych – kolumnę „Sten” pozostawia się pustą; interpretacja na podstawie punktów, średniej i poziomu wsparcia. KSzOF (szkoła) – steny wyłącznie w kolumnie „Sten” dla domen. Średniej nigdy nie opisuje się stenem.', { size:20, color:C.muted }) ], { before:80, after:0, line:250, align:AlignmentType.JUSTIFIED }) ];

// ---- 6 ----
const s6 = D.s6, CC = { red:C.red, blue:C.blue, purple:C.purple, orange:C.orange, green:C.green };
const part6 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Wyniki arkuszy specjalistycznych'), ...secL('6', s6.title, s6.law), lead2('', s6.lead),
...
const part7 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Mój głos'), ...secL('7', s7.title, s7.law), lead2('', s7.lead),
  ...
  P([ run('Podstawa. ', { size:20, bold:true, color:C.purple }), run(s7.podstawa, { size:20, color:C.muted }) ], { before:80, after:0 }) ];

// ---- 8 ----
const s8 = D.s8, W8 = [2000, 4300, 3606];
const part8 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Działania dotychczas podjęte'), ...secL('8', s8.title, s8.law), lead2(s8.ref, s8.lead),
...
const part9 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Analiza jakościowa'), ...secL('9', s9.title, s9.law), lead2('', s9.lead),
  tbl(W6, [ gridHead(['Domena ICF','Opis funkcjonowania i bariery','Cel na rok szkolny – co ma się zmienić'], W6, [C.purple, C.purple, C.orange]),
    ...s9.rows.map(r => row([ gcell([ txt(r[1], { bold:true, color:C.purple }), txt(r[0], { size:20, color:C.orange, bold:true }) ], W6[0]), gcell(txt(r[2]), W6[1]), gcell(bullets(r[3], '✓', C.green), W6[2]) ])) ]) ];

// ---- 10 ----
const s10 = D.s10, TW = Math.floor(CW/3);
const part10 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część II · Decyzja Zespołu'), ...secL('10', s10.title, s10.law), lead2('', s10.lead),
  sub9('A','Poziom wsparcia ustalony przez Zespół','',C.orange),
  tbl([TW,TW,TW], [ row(s10.poziomy.map(([k,t,d,on]) => cell([ P([ chk(on), run(t, { size:24, bold:true, color:C.purple }) ], { after:40 }), P([ run(d, { size:20, color:C.muted }) ], { after:0, line:240 }) ], { width:TW, bg: on ? C.orangeMist : C.white, borders:{ top:ln(on?C.orange:C.line), bottom:ln(on?C.orange:C.line), left:ln(on?C.orange:C.line), right:ln(on?C.orange:C.line) }, margins:{ top:80, bottom:80, left:160, right:140 } }))) ]),
  ...
  tbl([SWT,SWT,SWT,SWT], [ row(s10.wymiar.map(([k,v]) => cell([ P([ run(k.toUpperCase(), { size:20, bold:true, color:C.lavText, spacing:12 }) ], { after:30 }), P([ run(v, { size:24, bold:true, color:C.purple }) ], { after:0, line:240 }) ], { width:SWT, bg:C.lav, borders:{ top:ln(C.white,12), bottom:ln(C.white,12), left:ln(C.white,12), right:ln(C.white,12) }, margins:{ top:90, bottom:90, left:140, right:100 } }))) ]),
...
const part11 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Dostosowanie programu'),
  ...
  pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Organizacja i technologie'),
...
const part12 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Zintegrowane działania'), ...secL('12', s12.title, s12.law), lead2('', s12.lead),
...
const zajTable = (rows, edge, sumLabel, sumVal, sumSub) => tbl(WZ, [ gridHead(['Rodzaj zajęć','Zakres / cel','Prowadzący','Forma','Wymiar tyg.'], WZ),
  ...rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:24 }), WZ[0], { edge, pad:50 }), gcell(txt(r[1], { size:24 }), WZ[1], { pad:50 }), gcell(txt(r[2], { bold:true, color:C.purple, size:24 }), WZ[2], { pad:50 }), gcell(txt(r[3], { size:24 }), WZ[3], { pad:50 }), gcell([ txt(r[4], { bold:true, color:C.purple, size:24 }), txt(r[5], { size:20, color:C.muted }) ], WZ[4], { pad:50 }) ])),
  row([ cell(txt(sumLabel, { bold:true, color:C.purple, size:24 }), { width:WZ[0]+WZ[1]+WZ[2]+WZ[3], span:4, bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:80, bottom:80, left:140, right:120 } }), cell([ txt(sumVal, { bold:true, color:C.purple, size:24 }), txt(sumSub, { size:20, color:C.muted }) ], { width:WZ[4], bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:80, bottom:80, left:140, right:120 } }) ]) ]);
const pppTable = tbl(WP, [ gridHead(['Forma pomocy','Cel','Prowadzący','Forma i miejsce','Czas i termin','Okres udzielania'], WP),
  ...s13.ppp.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:20 }), WP[0], { edge:C.blue, pad:40 }), gcell(txt(r[1], { size:20 }), WP[1], { pad:40 }), gcell(txt(r[2], { bold:true, color:C.purple, size:20 }), WP[2], { pad:40 }), gcell(txt(r[3], { size:20 }), WP[3], { pad:40 }), gcell(txt(r[4], { bold:true, color:C.purple, size:20 }), WP[4], { pad:40 }), gcell(txt(r[5], { size:20 }), WP[5], { pad:40 }) ])),
  row([ cell(txt('Razem pomoc psychologiczno-pedagogiczna', { bold:true, color:C.purple, size:24 }), { width:CW-WP[5], span:5, bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:80, bottom:80, left:140, right:120 } }), cell([ txt(s13.sumPpp[0], { bold:true, color:C.purple, size:24 }), txt(s13.sumPpp[1], { size:20, color:C.muted }) ], { width:WP[5], bg:C.lav, borders:{ top:ln(), bottom:ln(), left:ln(), right:ln() }, margins:{ top:80, bottom:80, left:140, right:120 } }) ]) ]);
const statusCell = (w) => gcell(['wdrożone','w trakcie','planowane'].map(t => P([ run('☐ ', { size:24, color:'B6A6DF' }), run(t, { size:20, color:C.muted }) ], { after:0, line:220 })), w);
const part13 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Zajęcia: rewalidacja i PPP'), ...secL('13', s13.title, s13.law), lead2('', s13.lead),
  ...
  pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Pomoc psychologiczno-pedagogiczna'),
  sub9('B','Zajęcia z zakresu pomocy psychologiczno-pedagogicznej','forma · czas · termin · okres udzielania · miejsce', C.blue), pppTable,
  pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Realizacja zaleceń poradni'),
  P([ run('Uwaga: ', { size:20, bold:true, color:C.purple }), run(s13.uwaga, { size:20, color:C.muted }) ], { after:60 }),
  ...
  tbl(WM, [ gridHead(['Lp.','Zalecenie poradni (z orzeczenia / opinii)','Sposób realizacji w placówce','Gdzie w raporcie','Status'], WM, [C.purple, C.purple, C.orange, C.purple, C.purple]),
    ...s13.mapa.map((z,i) => row([ gcell(txt(String(i+1), { bold:true, color:C.orange, size:24 }), WM[0]), gcell(txt(z[0], { bold:true, size:24 }), WM[1]), gcell(txt(z[1], { size:24 }), WM[2]), gcell(P([ run(' '+z[2]+' ', { size:20, bold:true, color:C.orange, bg:C.orangeMist }) ], { after:0 }), WM[3]), statusCell(WM[4]) ])) ]) ];

// ---- 14 ----
const s14 = D.s14;
const part14 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Dodatkowa osoba'), ...secL('14', s14.title, s14.law),
  P(s14.rodzaj.flatMap(([t,on]) => [ chk(on), run(t+'      ', { size:24, bold:true, color:C.purple }) ]), { after:120 }),
  tbl([Math.floor(CW/3), CW-Math.floor(CW/3)], [ row([ cell([ P([ run('WYMIAR I SYTUACJE', { size:20, bold:true, color:C.lavText, spacing:12 }) ], { after:30 }), P([ run(s14.wymiar, { size:24, bold:true, color:C.purple }) ], { after:0, line:240 }) ], { width:Math.floor(CW/3), bg:C.lav, borders:noBorders, margins:{ top:90, bottom:90, left:140, right:100 } }), cell([ P([ run('PODSTAWA PRAWNA I FORMALNA', { size:20, bold:true, color:C.lavText, spacing:12 }) ], { after:30 }), P([ run(s14.podstawa, { size:20, color:C.purple }) ], { after:0, line:240 }) ], { width:CW-Math.floor(CW/3), bg:C.lav, borders:{ top:NOB, bottom:NOB, right:NOB, left:ln(C.white,12) }, margins:{ top:90, bottom:90, left:140, right:100 } }) ]) ]),
  ...
  P([ run('Wariant B: ', { size:20, bold:true, color:C.purple }), run(s14.wariantNote, { size:20, color:C.muted }) ], { before:80, after:0 }) ];

// ---- 15 ----
const s15 = D.s15, W15 = [2200, 3700, 2000, 2006];
const part15 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Współpraca z rodzicami i poradnią'), ...secL('15', s15.title, s15.law),
...
const part16 = [ pb(), ...pageHeader('Ocena Funkcjonalna · Część III · Ocena efektywności · podpisy'), ...secL('16', s16.title, s16.law), lead2('', s16.lead),
  tbl(W16, [ gridHead(['Termin','Zakres oceny','Narzędzia','Odpowiedzialny','Status'], W16),
    ...s16.rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:24 }), W16[0]), gcell(txt(r[1], { size:24 }), W16[1]), gcell(txt(r[2], { size:24 }), W16[2]), gcell(txt(r[3], { size:24 }), W16[3]), gcell(P([ run(' '+r[4]+' ', { size:20, bold:true, color:STC[r[4]].fg, bg:STC[r[4]].bg }) ], { after:0 }), W16[4]) ])) ]),
...
const okTxt = (v) => txt(v, { size:20, bold: v.startsWith('✓')||v.startsWith('—'), color: v.startsWith('✓') ? C.green : (v.startsWith('—') ? C.red : C.ink) });
const lawPage = [
  ...pageHeader('Ocena Funkcjonalna · Jak czytać raport · dwa warianty'),
  ...
  tbl([1981,1981,1981,1981,1982], [ row(PR.jakczytac.map((k,i) => cell([ P([ run('CZĘŚĆ '+k[0], { size:20, bold:true, color:C.orange, spacing:12 }) ], { after:20 }), P([ run(k[1], { size:24, bold:true, color:C.purple }) ], { after:30 }), P([ run(k[2], { size:20, color:C.muted }) ], { after:0, line:220 }) ], { width: i===4?1982:1981, margins:{ top:80, bottom:80, left:120, right:100 } }))) ]),
  section('A/B', WV.title), lead2('', WV.lead),
  tbl([half, CW-half], [ row([
    cell(P([ run(' A ', { size:20, bold:true, color:C.white, bg:C.orange }), run('  '+WV.A, { size:20 }) ], { after:0, line:250 }), { width:half, margins:{ top:100, bottom:100, left:160, right:140 } }),
    cell(P([ run(' B ', { size:20, bold:true, color:C.white, bg:C.blue }), run('  '+WV.B, { size:20 }) ], { after:0, line:250 }), { width:CW-half, margins:{ top:100, bottom:100, left:160, right:140 } })
  ]) ]),
  empty(100),
  tbl(WW, [ gridHead(['Sekcje','Zakres','Wariant A · z orzeczeniem','Wariant B · bez orzeczenia'], WW),
    ...WV.rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.orange, size:20 }), WW[0], { pad:45 }), gcell(txt(r[1], { size:20 }), WW[1], { pad:45 }), gcell(okTxt(r[2]), WW[2], { pad:45 }), gcell(okTxt(r[3]), WW[3], { pad:45 }) ])) ]),
  pb(),
  ...pageHeader('Ocena Funkcjonalna · Podstawy prawne'),
  section('§', PR.title), lead2('', PR.lead),
  tbl(WL, [ gridHead(['Akt prawny','Publikacja','Zakres zastosowania w raporcie'], WL),
    ...PR.rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:20 }), WL[0], { pad:50 }), gcell(txt(r[1], { bold:true, color:C.orange, size:20 }), WL[1], { pad:50 }), gcell(P([ run(r[2]+'  ', { size:20 }), run(' '+r[3]+' ', { size:20, bold:true, color:C.orange, bg:C.orangeMist }) ], { after:0, line:230 }), WL[2], { pad:50 }) ])) ]),
...
const opHead = () => [ tbl([half, CW-half], [ row([
    cell([ P([ run(PL.nazwa, { size:24, bold:true, color:C.purple }) ], { after:20 }), P([ run(PL.adres, { size:20, color:C.muted }) ], { after:20 }), P([ run('(pieczęć placówki)', { size:20, color:C.muted }) ], { after:0 }) ], { width:half, borders:{ top:{ style:BorderStyle.DASHED, size:20, color:C.line }, bottom:{ style:BorderStyle.DASHED, size:20, color:C.line }, left:{ style:BorderStyle.DASHED, size:20, color:C.line }, right:{ style:BorderStyle.DASHED, size:20, color:C.line } }, margins:{ top:120, bottom:120, left:180, right:180 } }),
    cell([ P([ run(PL.miejscowosc+', dnia ………………………', { size:20, color:C.muted }) ], { align:AlignmentType.RIGHT, after:40 }), P([ run('Znak sprawy: ………………………', { size:20, color:C.muted }) ], { align:AlignmentType.RIGHT, after:40 }), P([ run('Zespół orzekający: [Nazwa Poradni Psychologiczno-Pedagogicznej]', { size:20, color:C.muted }) ], { align:AlignmentType.RIGHT, after:0 }) ], { width:CW-half, borders:noBorders, margins:{ top:60, bottom:60, left:200, right:0 } })
  ]) ]), empty(160) ];
const opSec = (n, t) => new Paragraph({ spacing:{ before:180, after:100 }, keepNext:true, border:{ bottom:{ style:BorderStyle.SINGLE, size:20, color:C.line, space:4 } }, children:[ run(n+'  ', { size:24, bold:true, color:C.orange }), run(t.toUpperCase(), { size:24, bold:true, color:C.purple, spacing:8 }) ] });
const opKV = (rows) => tbl([3000, CW-3000], rows.map(([k,v]) => row([ gcell(txt(k, { bold:true, color:C.purple, size:20 }), 3000, { bg:C.lav2, pad:55 }), gcell(txt(v.replace(/☐/g,'☐'), { size:20 }), CW-3000, { pad:55 }) ])));
...
const opICF = (rows) => tbl(WO, [ gridHead(['Obszar (ICF)','Mocne strony i uzdolnienia','Trudności','Poziom potrzeby wsparcia'], WO, [C.purple, C.green, C.red, C.purple]),
  ...rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:20 }), WO[0], { bg:C.lav2, pad:45 }), gcell(txt(r[1], { size:20 }), WO[1], { pad:45 }), gcell(txt(r[2], { size:20 }), WO[2], { pad:45 }), gcell(txt(r[3], { bold:true, color:C.purple, size:20 }), WO[3], { pad:45 }) ])) ]);
const opBul = (items, color=C.orange) => items.map(x => new Paragraph({ spacing:{ before:0, after:40, line:245 }, indent:{ left:220, hanging:220 }, children:[ run('•  ', { size:20, color, bold:true }), run(x, { size:20 }) ] }));
...
const opinionPages = [
  ...
  P([ run(OP.sub.toUpperCase(), { size:20, bold:true, color:C.orange, spacing:30 }) ], { align:AlignmentType.CENTER, after:140 }),
  tbl([CW], [ row([ cell([ P([ run('Podstawa prawna: ', { size:20, bold:true, color:C.purple }), run(OP.podstawa, { size:20, color:C.muted }) ], { after:40, line:240, align:AlignmentType.JUSTIFIED }), P([ run('Tryb: ', { size:20, bold:true, color:C.purple }), run(OP.procedura, { size:20, color:C.muted }) ], { after:0, line:240, align:AlignmentType.JUSTIFIED }) ], { width:CW, margins:{ top:100, bottom:100, left:180, right:180 } }) ]) ]),
  ...
  P([ run(OP.II.wynik, { size:20, bold:true, color:C.purple }) ], { before:80, after:0 }),
  opSec('4.', OP.III.title), ...opBul(OP.III.items),
  pb(), ...opHead(),
  opSec('5.', OP.zal.title), ...OP.zal.items.map(([t,on]) => P([ chk(on), run(t, { size:20 }) ], { after:40 })),
  opSec('6.', OP.IV.title),
  tbl(WI, [ gridHead(['Działanie / forma pomocy','Zakres i wymiar','Okres udzielania','Efekty'], WI), ...OP.IV.rows.map(r => row([ gcell(txt(r[0], { bold:true, color:C.purple, size:20 }), WI[0], { bg:C.lav2, pad:50 }), gcell(txt(r[1], { size:20 }), WI[1], { pad:50 }), gcell(txt(r[2], { size:20 }), WI[2], { pad:50 }), gcell(txt(r[3], { size:20 }), WI[3], { pad:50 }) ])) ]),
  opSec('7.', OP.VI.title), ...opBul(OP.VI.dalsza, C.green),
  pb(), ...opHead(),
  opSec('8.', OP.uzup.title), lead2('', OP.uzup.lead),
  P([ run(OP.uzup.funkcje, { size:20 }) ], { after:100, line:240, align:AlignmentType.JUSTIFIED }),
  ...
  P([ run('Informacje od dziecka i rodziców (§ 8 ust. 3 pkt 1 i 3): ', { size:20, bold:true, color:C.purple }), run(OP.uzup.glos, { size:20 }) ], { before:80, after:60, line:240, align:AlignmentType.JUSTIFIED }),
  sub9('!', OP.uzup.rekTitle, '', C.blue), cbGrid(OP.VI.items.map(([t,on]) => [t,on])),
  P([ run('Uzasadnienie: ', { size:20, bold:true, color:C.purple }), run(OP.VI.uzasadnienie, { size:20 }) ], { before:40, after:0, line:240, align:AlignmentType.JUSTIFIED }),
  opSec('9.', OP.podpisy.osoby),
  tbl([half, CW-half], (() => { const rows=[]; for (let i=0;i<OP.sporzadzili.length;i+=2) rows.push(row([0,1].map(j => { const it=OP.sporzadzili[i+j]; return cell(it ? [ P([ run(it[0]+': ', { size:20, bold:true, color:C.purple }), run(it[1], { size:20 }) ], { after:10 }), P([ run('podpis: ………………………………………', { size:20, color:C.muted }) ], { after:0 }) ] : [empty()], { width:half, borders:noBorders, margins:{ top:40, bottom:40, left:0, right:200 } }); }))); return rows; })()),
  tbl([Math.floor(CW/2), CW-Math.floor(CW/2)], [ row([ sigCell(OP.podpisy.dyrektor+' · podpis i pieczęć · data', Math.floor(CW/2), undefined, 360), cell([ P([ run(OP.podpisy.kopia, { size:20, color:C.muted }) ], { after:0, line:240 }) ], { width:CW-Math.floor(CW/2), borders:noBorders, vAlign:VerticalAlign.BOTTOM, margins:{ top:360, bottom:20, left:220, right:120 } }) ]) ]),
  P([ run(OP.zalaczniki+' Opinia zawiera dane dotyczące zdrowia (art. 9 RODO) – dokument poufny.', { size:20, color:C.muted }) ], { before:100, after:0, line:240, border:{ top:{ style:BorderStyle.DASHED, size:20, color:C.line, space:6 } } })
];
const opinionFooter = new Paragraph({
  spacing:{ before:60, after:0 }, border:{ top:{ style:BorderStyle.SINGLE, size:20, color:C.line2, space:4 } },
  tabStops:[{ type:TabStopType.RIGHT, position:CW }],
  children:[ run('Załącznik – opinia o funkcjonowaniu dziecka / ucznia · dokument poufny (RODO)', { size:20, color:C.muted }), run('\t'), run('Strona ', { size:20, color:C.muted }), new TextRun({ children:[PageNumber.CURRENT], font:FONT, size:20, bold:true, color:C.orange }), run(' z ', { size:20, color:C.muted }), new TextRun({ children:[PageNumber.TOTAL_PAGES], font:FONT, size:20, bold:true, color:C.purple }) ]
});

const children = [
  ...cover,
  ...lawPage,
  ...pageHeader('Ocena Funkcjonalna · Metryczka i obserwacja wstępna'),
  ...
  icfRow,
  new Paragraph({ children:[ new PageBreak() ] }),
  ...pageHeader('Ocena Funkcjonalna · Obserwacja pogłębiona'),
