...
def _dzwiek(nr):
    """Ścieżka narracji nr jako data-URI, a gdy pliku nie ma — pusty ciąg.

    Nagrania jej głosu nie wchodzą do repozytorium (dana biometryczna), więc
    na świeżym klonie tego pliku nie ma. Budowanie ma się nie wywalić: karta
    złoży się bez dźwięku, tak samo jak karta pomocy bez nagrania.
    """
    p = _AUDIO / NARRACJA[nr][0]
    if not p.exists():
        return ""
...
def audio_scen():
    ...
    return "".join(f'<audio id="au{n}" preload="none" src="{zrodlo}"></audio>'
                   for n in NARRACJA if (zrodlo := _dzwiek(n)))
