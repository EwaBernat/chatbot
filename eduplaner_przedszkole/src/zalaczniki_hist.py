...
def _dzwiek(h, nr):
    """Narracja jako data: URI, a gdy pliku nie ma — pusty ciąg.

    Nagrania są jej sklonowanym głosem i nie wchodzą do repozytorium, bo to
    dana biometryczna. Wczytywanie ich na twardo wysypywało budowanie całego
    banku na świeżym klonie — a zasada tego modułu jest odwrotna: brak nie
    wysypuje budowania, tylko zostawia wyłączony przycisk.
    """
    p = _ASSETS / h["audio"] / PLIKI[nr]
    if not p.exists():
        return ""
    return f"data:audio/mpeg;base64,{base64.b64encode(p.read_bytes()).decode()}"
...
def _audio(pre, h):
    return "".join(f'<audio id="{pre}au{n}" preload="none" src="{zrodlo}"></audio>'
                   for n in PLIKI if (zrodlo := _dzwiek(h, n)))
