...
DANE = KORZEN / "01_dane_json" / "cele_mowa_obserwacja.json"
...
def naglowek(sub: str, uczen: str, grupa: str) -> str:
  ...
  <div class="prawa"><b>MOWA · WOPF</b><span>narzędzie · druk MOWA-C</span></div></div>
...
def strona_wstepna(dane: dict, wyniki: dict | None, uczen: str, grupa: str, ile: int) -> str:
    wiersze = []
    for nr in KOLEJNOSC:
        z = next(x for x in dane["obszary"] if x["nr"] == nr)
        if wyniki:
            suma = wyniki[nr]
...
{naglowek('Rozwój mowy i komunikacja · cele SMART — wprowadzenie', uczen, grupa)}
<div class="tresc">
  ...
  <div class="dwie">
    ...
    <div class="karta-info"><h3>Skąd kryterium i horyzont</h3>
      Nie z podręcznika, tylko z <b>wyniku tego obszaru u tego dziecka</b>. Uwaga: tu im wyżej,
      tym lepiej. Obszar z wynikiem 0–3 (znaczne trudności) jest priorytetem, więc sprawdzamy go
      ...
      przy 3 z 5. Ta sama logika, co przy poziomach wsparcia w druku MOWA-T.</div>
  </div>
  <table><thead><tr><th>Obszar</th><th>Wynik</th><th>Norma rozwojowa (typowy wiek)</th>
    <th>Pasmo</th><th>Kryterium</th><th>Horyzont</th><th>Znaczenie dla planu pracy</th></tr></thead>
    <tbody>{''.join(wiersze)}</tbody></table>
  <div class="uwagi">{e(dane['przelicznik_natezenia'])} — 5 obszarów × 5 pozycji × skala 0–2.
    Rozbieżność między normą rozwojową a wynikiem wskazuje, na których obszarach skupić pracę.</div>
  <table><thead><tr><th>Wynik obszaru</th><th>Pasmo</th><th>Kryterium</th><th>Horyzont</th>
    <th>Znaczenie</th></tr></thead><tbody>{progi}</tbody></table>
  <div class="uwagi"><b>Nie wszystkie 25 celów naraz.</b> Do planu bierzemy cele z obszaru
    priorytetowego i jeden–dwa z obszaru wymagającego wsparcia; 25 celów to zapas na cały rok,
    nie plan na wrzesień. <b>Kolejność jest rozwojowa</b>: obszary IV i V mają sens dopiero
...
CELE_NA_STRONE = 3   # tyle bloków celu mieści się na A4 obok nagłówka obszaru


def strona_obszaru(z: dict, suma: int | None, prog: dict | None, uczen: str, grupa: str,
  ...
  <p class="strat">krok komunikacyjny: {e(w['krok_komunikacyjny'])} · pozycja karty: {e(w['pozycja'])}</p>
...
{stopka(nr_strony, ile, f"obszar {z['nr']} · {z['nazwa'].lower()}")}
</section>"""
...
def strona_ewaluacji(dane: dict, uczen: str, grupa: str, ile: int) -> str:
    wiersze = "".join(
        ...
        for z in dane["obszary"])
    return f"""<section class="strona">
{naglowek('Rozwój mowy · ewaluacja', uczen, grupa)}
<div class="tresc">
  <div class="pas">Ewaluacja — postęp wg obszarów (0–10 pkt)</div>
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Druk MOWA-C — cele SMART do obserwacji pogłębionej")
    ...
    ap.add_argument("--wyniki", default="", help="5 wyników obszarów 0–10 po przecinku, w kolejności karty")
    args = ap.parse_args()
    ...
    if args.wyniki:
        ...
        if len(liczby) != 5:
            raise SystemExit("--wyniki wymaga dokładnie 5 liczb (5 obszarów), 0–10 każda.")
        if any(not 0 <= n <= 10 for n in liczby):
            raise SystemExit("Wynik obszaru mieści się w skali 0–10.")
        wyniki = dict(zip(KOLEJNOSC, liczby))

    # Obszar ma pięć wskaźników, a na A4 mieszczą się trzy bloki celu. Reszta
    ...
    for i, (z, nr, czesc, dalej) in enumerate(porcje, start=2):
        ...
        strony.append(strona_obszaru(z, suma, prog, args.uczen, args.grupa, i, ile,
                                        wskazniki=czesc, dalszy_ciag=dalej))
    strony.append(strona_ewaluacji(dane, args.uczen, args.grupa, ile))

    nazwa = (f"uczen_{slug(args.uczen)}_MOWA-C.html" if args.uczen
             else "Cele_SMART_MOWA_obserwacja_poglebiona.html")
    wyjscie = KATALOG / nazwa
    KATALOG.mkdir(parents=True, exist_ok=True)
    tytul = ("Cele SMART · rozwój mowy"
