# Kod źródłowy modułu rozwoju mowy
...
dane_zrodlowe.py ──► eksport_json.py ──► 01_dane_json/*.json   ← to się wpina do aplikacji
                                              │
                                              ├─► build_tabela.py     ──► MOWA-T (HTML)
                                              ├─► build_cele_mowa.py ──► MOWA-C (HTML)
...
| `OBSZARY` | 5 obszarów karty mowy: ICF, podstawa programowa, norma rozwojowa, `zasada_mowy` |
| `PROGI` | pasma wyniku obszaru (0–10) → kryterium i horyzont dla druku MOWA-C |
| `POZIOMY` | p3 / p2 / p1 z kryterium, horyzontem i warunkami |
| `WERSJE` | A / B / C — wiek, czas zajęć, forma, cykl, język poleceń |
| `PRZELICZNIK` | opis skali: 5 obszarów × 5 pozycji × skala 0–2 |
...
```python
{
  ...
  "wskaznik": "…",                       # pozycja karty obserwacji rozwoju mowy
  "krok_komunikacyjny": "…",             # SEDNO — odpowiednik zachowania zastępczego z FBA
  "opis_kroku": "…",
...
|---|---|---|
| `historyjka` | pola numerowane, czyta się w prawo | spacer po kasztany w czterech polach (V.3) |
| `rozgalezienie` | początek → dwa zakończenia, wiersz na historię | karta i płacz przy tej samej półce (V.2) |
| `listwa` | pola scenariusza do powieszenia | odkładam → idę → siadam (I.3) |
| `zestaw` | pola niezależne, podawane po jednym | słowa do klaskania (III.3), czynności (IV.2) |
...
## Granica kompetencji — czego w tym banku NIE MA

Bank jest dla nauczyciela i pedagoga, nie dla logopedy. Korekcja artykulacji, usprawnianie
aparatu mowy i terapia jąkania **nie mają tu celów** — przy wskaźnikach II.3, II.4 i III.5
cele dotyczą warunków porozumienia się i częstości wypowiedzi, a nie brzmienia głosek.
Wynik 0 albo 1 na tych pozycjach jest przesłanką do skierowania na diagnozę logopedyczną.
Stała `MODUL["granica_kompetencji"]` trzyma to zdanie w danych, a nie tylko w tym pliku,
żeby weszło do każdego dokumentu, który je wyświetla.

## Skąd bierze się kryterium

**Druk MOWA-T** — z poziomu wsparcia:
...
**Druk MOWA-C** — z wyniku obszaru u konkretnego dziecka (0–10; tu im wyżej, tym lepiej):
...
## Druk MOWA-T — co robi JavaScript
...
* druk jednego konspektu (A4 pionowo) i całego zeszytu 25 konspektów danej wersji;
* przycisk „+” w komórce otwiera formularz własnego konspektu; rekord zapisuje się
  w `localStorage` pod kluczem `eduplaner2026.moje-konspekty-mowa.v1`, w kształcie
...
python3 build_tabela.py               # MOWA-T: tabela 225 celów + 75 konspektów
python3 build_cele_mowa.py           # MOWA-C: 12 stron A4 do wypełnienia
python3 build_cele_mowa.py --uczen "…" --grupa "…" --wyniki 8,6,3,1,2
...
do aplikacji trafił cel bez kroku komunikacyjnego.
