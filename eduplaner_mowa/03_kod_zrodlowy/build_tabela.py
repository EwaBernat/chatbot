#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Druk MOWA-T — tabela 225 celów SMART, z której wchodzi się w konspekty.
...
WYJSCIE = KORZEN / "02_gotowe_dokumenty" / "Tabela_celow_MOWA_wiek_poziom.html"
...
def naglowek(liczba_celow: int) -> str:
  ...
  <div><h1>EduPlaner 2026</h1>
    <div class="sub">Rozwój mowy i komunikacja · tabela celów SMART · wiek i poziom wsparcia</div></div>
  <div class="prawa"><b>MOWA · WOPF</b><span>narzędzie · druk MOWA-T</span></div>
</div>
...
def uwaga(opis_modulu: str) -> str:
    return f"""<div class="uwaga"><b>Poziom zmienia warunki, nie krok komunikacyjny.</b>
...
def wykaz_konspektow(wersja: str, obszary: list[dict], konspekty_wg: dict) -> str:
    grupy = []
    for z in obszary:
        karty = "".join(
            ...
            f'<span class="kzast">{e(w["krok_komunikacyjny"])}</span></button>'
            for w in z["wskazniki"])
        grupy.append(f'<div class="kgrupa"><h4>Obszar {e(z["nr"])} · {e(z["nazwa"])} '
...
def tabela_wersji(wersja: str, wiek: str, obszary: list[dict], poziomy: list[dict],
                  konspekty_wg: dict) -> str:
    wiersze = []
    for z in obszary:
        wiersze.append(f'<tr class="pas"><td colspan="5">Obszar {e(z["nr"])} · {e(z["nazwa"])}'
        ...
        for w in z["wskazniki"]:
            ...
            wiersze.append(
                f'<tr data-wsk="{e(w["nr"])}" data-obszar="{e(z["nr"])}" '
                f'data-krok="{e(w["krok_komunikacyjny"])}">'
                f'<td class="nr">{e(w["nr"])}</td>'
                f'<td class="wsk"><b>{e(w["wskaznik"])}</b>'
                f'<span>krok komunikacyjny: {e(w["krok_komunikacyjny"])}</span>'
  ...
  {wykaz_konspektow(wersja, obszary, konspekty_wg)}
  <table>
    ...
    <thead>
      <tr class="wband"><th colspan="5">EduPlaner 2026 · druk MOWA-T · cele SMART do wskaźników
        rozwoju mowy <b>wersja {wersja} · {e(wiek)}</b></th></tr>
      <tr><th>Nr</th><th>Wskaźnik z karty obserwacji MOWA</th>{naglowki}</tr>
    </thead>
...
def modal_konspektu(kon: dict, pomoc: dict, arkusz: dict, wskaznik: dict, poziomy: list[dict]) -> str:
  ...
  <div class="kcard">
    ...
    <div class="kcele">
      <div class="kcel edu"><div class="kchead">Cel edukacyjny — z tabeli MOWA-T, wg klikniętego poziomu</div>
        {warianty}</div>
...
function kontekst(td){
  ...
  return {wersja: td.dataset.wersja, nr: td.dataset.wsk, poziom: td.dataset.lvl,
          obszar: tr.dataset.obszar, krok: tr.dataset.krok,
...
function formularz(k, rekord){
  ...
  const m = modal('mkt-form', `
    ...
    <div class="khead"><span class="mark" role="img" aria-label="Logo PCTP"></span>
      <div><div class="kw">EduPlaner 2026</div>
        <div class="ks">Mój konspekt · obszar ${esc(k.obszar)} · ${esc(OBSZARY_JS[k.obszar] || '')}
    ...
    <div class="kkrok"><b>Cel z tabeli (${esc(poz.nazwa)}):</b> ${esc(t.cel)}
      <span>${esc(t.ram)} · krok komunikacyjny: ${esc(k.krok)}</span></div>
  ...
  m.querySelector('#mkt-formularz').addEventListener('submit', ev => {
    ...
    const rek = {
      ...
      nr: k.nr, wersja: k.wersja, poziom: k.poziom, obszar: k.obszar, krok: k.krok,
...
function pokazWlasny(id){
  ...
  modal('mkt-widok', `
    ...
    <div class="khead"><span class="mark" role="img" aria-label="Logo PCTP"></span>
      <div><div class="kw">EduPlaner 2026</div>
        <div class="ks">Konspekt własny · obszar ${esc(r.obszar)} · ${esc(OBSZARY_JS[r.obszar] || '')}
    ...
    <div class="ktitle"><span class="kp">Konspekt zajęć · druk KC-3 · opracowanie własne</span>
      <div class="ksfera">Wskaźnik ${esc(r.nr)} · krok komunikacyjny: ${esc(r.krok)}</div>
    ...
    <div class="ksec"><span class="sq">I</span><h4>Cel SMART</h4><span class="line"></span></div>
    <div class="kcele">
      <div class="kcel edu"><div class="kchead">Cel edukacyjny — z tabeli MOWA-T · ${esc(poz.nazwa)}</div>
  ...
  document.getElementById('mkt-edytuj').addEventListener('click', () => {
    zamknij();
    formularz({wersja: r.wersja, nr: r.nr, poziom: r.poziom, obszar: r.obszar,
...
def main() -> int:
    cele = wczytaj("cele_mowa_poziomy.json")
    konspekty = wczytaj("konspekty_mowa.json")
    pomoce = wczytaj("pomoce_mowa.json")
    materialy = wczytaj("materialy_do_druku.json")

    poziomy, wersje, obszary = cele["poziomy_wsparcia"], cele["wersje_wiekowe"], cele["obszary"]
    ...
    wskaznik_wg = {w["nr"]: w for z in obszary for w in z["wskazniki"]}

    sekcje = "".join(tabela_wersji(w["klucz"], w["wiek"], obszary, poziomy, konspekty_wg) for w in wersje)
    modale = "".join(
    ...
    skrypt = (SKRYPT
              .replace("__POZIOMY__", json.dumps({p["klucz"]: {"nazwa": p["nazwa"], "rzym": p["rzym"],
                                                 ...
                                                 ensure_ascii=False))
              .replace("__OBSZARY__", json.dumps({z["nr"]: z["nazwa"] for z in obszary}, ensure_ascii=False))
...
<title>Tabela celów SMART · rozwój mowy (przedszkole) — EduPlaner 2026 · PCTP</title>
<style>{STYL}
...
{sekcje}
  <div class="stopka">
    ...
    <span>druk MOWA-T · tabela drukuje się poziomo · {cele['liczba_celow']} celów ·
