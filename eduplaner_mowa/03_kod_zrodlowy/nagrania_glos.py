...
Buduje manifest nagrań z `pomoce_mowa.json` i — na żądanie — generuje pliki MP3
...
GŁOS JEST DANĄ BIOMETRYCZNĄ. Pliki z katalogu audio_mowa/ to sklonowany głos
...
POMOCE = KORZEN / "01_dane_json" / "pomoce_mowa.json"
MANIFEST = KORZEN / "01_dane_json" / "nagrania_mowa.json"
...
def zapisz_manifest(lista: list[dict]) -> None:
    MANIFEST.write_text(json.dumps({
        "opis": ("Manifest nagrań poleceń dla dziecka — rozwój mowy (MOWA). Ścieżki liczone od katalogu 04_media/, tak jak w module ABC/FBA."),
        "uwaga_prawna": (
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Nagrania poleceń dla dziecka — rozwój mowy (MOWA)")
